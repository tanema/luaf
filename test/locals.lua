...
do
  ...
  do
    ...
    do
      ...
      local x <close> = setmetatable({ "x" }, {
        __close = function(self) a[#a + 1] = self[1] end,
      })
  ...
  do
    ...
    local function foo()
      return function() return true end, 0, 0, func2close(function() closed = true end)
    end
  ...
  do
    ...
    local function foo()
      ...
      local x1 <close> = func2close(function(self, msg) assert(string.find(msg, "@y")) end)
  ...
  if rawget(_G, "T") then
    ...
    local function test()
      ...
      local xx <close> = func2close(function(self, msg) assert(msg == "not enough memory") end)
      local x <close> = enter(0) -- set a memory limit
      local y = {} -- raise a memory error
    end
    ...
    do
      ...
      local function hook(event) trace[#trace + 1] = event .. " " .. (debug.getinfo(2).name or "?") end

      -- create tbc variables to be used by C function
      local x = func2close(function(_, msg) trace[#trace + 1] = "x" end)

      local y = func2close(function(_, msg) trace[#trace + 1] = "y" end)
  ...
  do -- '__close' vs. return hooks in Lua functions
    ...
    local function foo(...)
      local x <close> = func2close(function(_, msg) trace[#trace + 1] = "x" end)

      local y <close> = func2close(function(_, msg) debug.sethook(hook, "r") end)
