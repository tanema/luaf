...
assert(0e12 == 0 and 0.0 == 0 and 0. == 0 and 0.2e2 == 20 and 2.E-1 == 0.2)
...
do
  ...
  for i = -3, 3 do -- variables avoid constant folding
    for j = -3, 3 do
      ...
      if not _port or i ~= 0 or j > 0 then assert(eq(i ^ j, 1 / i ^ -j)) end
    end
  end
end
...
local function checkcompt(msg, code) checkerror(msg, assert(load(code))) end
...
assert(
  tonumber("+0.01") == 1 / 100
    and tonumber("+.01") == 0.01
    and tonumber(".01") == 0.01
    and tonumber("-1.") == -1
    and tonumber("+1.") == 1
)
assert(
  not tonumber("+ 0.01") and not tonumber("+.e1") and not tonumber("1e") and not tonumber("1.0e+") and not tonumber(".")
)
...
assert(1.1 == 1. + 0.1)
assert(100.0 == 1E2 and 0.01 == 1e-2)
...
for i = -10, 10 do
  for j = -10, 10 do
    if j ~= 0 then assert((i + 0.0) % j == i % j) end
  end
end

for i = 0, 10 do
  for j = -10, 10 do
    if j ~= 0 then assert((2 ^ i) % j == (1 << i) % j) end
  end
end
...
for i = -6, 6 do
  for j = -6, 6 do
    if j ~= 0 then
      ...
      if (i >= 0 and j >= 0) or (i <= 0 and j <= 0) or mi == 0 then assert(eqT(mi, i % j)) end
    end
  end
end
...
local function testnear(val, ref, tol) return (math.abs(val - ref) < ref * tol) end
...
do -- test random for floats
  ...
  if not (eq(up, 1, 0.001) and eq(low, 0, 0.001)) then goto doagain end
  -- all bit counts should be near 50%
  local expected = (totalrounds / randbits / 2)
  for i = 1, randbits do
    if not testnear(counts[i], expected, 0.10) then goto doagain end
  end
...
do -- test random for full integers
  ...
  totalrounds = totalrounds + rounds
  local lim = maxint >> 10
  if not (maxint - up < lim and low - minint < lim) then goto doagain end
  -- all bit counts should be near 50%
...
do
  ...
  for i = 1, 6 do
    if not testnear(count[i], totalrep, 0.05) then goto doagain end
  end
end
...
do
  local function aux(x1, x2) -- test random for small intervals
    local mark = {}
    local count = 0 -- to check that all values appeared
    while true do
...
do
  local function aux(p1, p2) -- test random for large intervals
    ...
    local n = 100
    local mark = {}
    local count = 0 -- to count how many different values
    ::doagain::
    for _ = 1, n do
    ...
    if not (count >= n * 0.8) then goto doagain end
    -- min and max not too far from formal min and max
    local diff = (p2 - p1) >> 4
    if not (min < p1 + diff and max > p2 - diff) then goto doagain end
  end
  aux(0, maxint)
  aux(1, maxint)
  aux(3, maxint // 3)
...
print(string.format("  %d-digit float numbers with full precision", decdig))
-- number of decimal digits supported by integer precision
local Idecdig = math.floor(math.log(maxint, 10))
print(string.format("  %d-digit integer numbers with full precision", Idecdig))
