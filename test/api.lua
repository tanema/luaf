...
-- testing MULTRET
t = pack(T.testC("call 2,-1; return *", function(a, b)
	return 1, 2, 3, 4, a, b
end, "alo", "joao"))
...
do -- testing multipe returns
	...
	local t = { T.testC(
		[[
    checkstack 300 "error"; call 1 250; return 250]],
		foo,
		250
	) }
...
assert(to("topointer", string.rep("x", 10)) == to("topointer", string.rep("x", 10))) -- short strings
do -- long strings
...
do
	...
	assert(T.checkpanic("loadstring 4 name bt") == "bad argument #4 (string expected, got no value)")
...
do
	...
	X = 0
	code()
	assert(X == N and Y == string.rep("a", N))
	X = nil
	Y = nil
...
-- doing nothing
b = testamem("doing nothing", function()
	return 10
end)
assert(b == 10)
...
testamem("gsub", function()
	local a, b = string.gsub("alo alo", "(a)", function(x)
		return x .. "b"
	end)
	return (a == "ablo ablo")
end)
