...
function stringsTest.testStringUpper() t.assert.Eq("AB\0C", string.upper("ab\0c")) end

function stringsTest.testStringLower() t.assert.Eq("\0abcc%$", string.lower("\0ABCc%$")) end
...
function stringsTest.testStringGMatch()
  ...
  local function isbalanced(s) return not string.find(string.gsub(s, "%b()", ""), "[()]") end
  ...
  t.assert.Error(function() string.gsub("alo", ".", { a = {} }) end, "invalid replacement value")
  t.assert.Error(function() string.gsub("alo", "(%0)", "a") end, "invalid capture index")
  t.assert.Error(function() string.gsub("alo", "(%1)", "a") end, "invalid capture index")
  t.assert.Error(function() string.gsub("alo", ".", "%x") end, "invalid use of")
  ...
  setmetatable(replTbl, {
    __index = function(_, s) return string.upper(s) end,
  })
...
function stringsTest.testPack()
  ...
  t.assert.Error(function() return string.pack("i0", 0) end, "out of limits")
  t.assert.Error(function() return string.pack("i" .. NB + 1, 0) end, "out of limits")
  t.assert.Error(function() return string.pack("!" .. NB + 1, 0) end, "out of limits")
  t.assert.Error(function() return string.pack("Xi" .. NB + 1, 0) end, "%(17%) out of limits %[1,16%]")
  t.assert.Error(function() return string.pack("i3r", 0) end, "invalid format option 'r'")
  t.assert.Error(function() return string.pack("!4i3", 0) end, "not power of 2")
  t.assert.Error(function() return string.pack("c", "") end, "missing size")
  t.assert.Error(function() return string.unpack("i16", string.rep("\3", 16)) end, "16%-byte integer")
  t.assert.Error(function() return string.packsize("s") end, "variable%-length format")
  t.assert.Error(function() return string.packsize("z") end, "variable%-length format")

  -- overflow in option size  (error will be in digit after limit)
  t.assert.Error(function() return string.packsize("c1" .. string.rep("0", 40)) end, "invalid format")
  ...
  do
    ...
    t.assert.Error(function() return string.pack("s1", s) end, "does not fit")
    t.assert.Error(function() return string.pack("z", "alo\0") end, "contains zeros")
    t.assert.Error(function() return string.unpack("zc10000000", "alo") end, "unfinished string")
    for i = 2, NB do
  ...
  do
    ...
    t.assert.Error(function() return string.unpack("s", x:sub(1, -2)) end, "too short")
    t.assert.Error(function() return string.unpack("c5", "abcd") end, "too short")
    t.assert.Error(function() return string.pack("s100", "alo") end, "out of limits")
  end
  ...
  do
    ...
    t.assert.Eq("xyz", b)
    t.assert.Eq(14, c)
    t.assert.Error(function() return string.pack("c3", "1234") end, "longer than")
  end
  ...
  do
    ...
    t.assert.Error(function() return string.pack("X") end, "invalid next option")
    t.assert.Error(function() return string.pack("XXi", "") end, "invalid next option")
    t.assert.Error(function() return string.pack("X i", "") end, "invalid next option")
    t.assert.Error(function() return string.pack("Xci", "") end, "invalid next option")
  end
  ...
  do -- testing initial position
    ...
    t.assert.Error(function() return string.unpack("c0", x, #x + 2) end, "out of string")
  end
end
