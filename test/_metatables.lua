...
metaTbl.__add = function(lval, rval) return lval.val + rval.val end

metaTbl.__eq = function(lval, rval) return lval.val == rval.val end

metaTbl.__le = function(lval, rval) return lval.val < rval.val end

metaTbl.__index = function() return "goop" end
...
function metaTableTests.testMetaMethods()
  ...
  local didCall = false
  local callmt = {
    __call = function() didCall = true end,
  }
...
function metaTableTests.testFormatToString()
  ...
  t.assert.Error(function() print(tostring(m)) end, "'__tostring' must return a string")
end
