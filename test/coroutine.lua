...
do
  ...
  local main = coroutine.running(); -- cannot close a "normal" coroutine
...
co = coroutine.wrap(function() coroutine.yield(xpcall(f, g, 10, 20)) end)
...
if not _soft then
  ...
  for i = 1, #t do
    local j = t[i]
    local co = coroutine.create(function() return table.unpack({}, 1, j) end)
...
assert(run(function() return a .. b .. c .. a end, { "concat", "concat", "concat" }) == "1012hello10")

assert(
  run(function() return "a" .. "b" .. a .. "c" .. c .. b .. "x" end, { "concat", "concat", "concat" })
    == "ab10chello12x"
)
...
local function apico(...)
  local x = { ... }
  return coroutine.wrap(function() return T.testC(table.unpack(x)) end)
end
