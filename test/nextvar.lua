...
if not T then
  ...
  local sizes = { 0, 1, 2, 3, 4, 5, 7, 8, 9, 15, 16, 17, 30, 31, 32, 33, 34, 254, 255, 256, 257, 500, 1001 }
...
a = {}
for i = 0, 10000 do
  if math.fmod(i, 10) ~= 0 then a["x" .. i] = i end
end
...
do -- clear global table
  ...
  for n, v in pairs(a) do
    if not package.loaded[n] and type(v) ~= "function" and not string.find(n, "^[%u_]") then _G[n] = undef end
    collectgarbage()
  end
end
...
if not T then
  ...
  debug.setmetatable(u2, {
    __newindex = function(_, k, v) tab2[k] = v end,
  })
...
setmetatable(a, {
  __pairs = function(x) return foo, x, 0 end,
})
