...
do
	...
	assert(
		b.name == nil
			and b.what == "Lua"
			and b.linedefined == testline
			and b.lastlinedefined == b.linedefined + 10
			and b.func == test
			and not string.find(b.short_src, "%[")
	)
...
do -- testing active lines
	...
	checkactivelines(function(a, b) end, { 1 })
...
foo(200, 3, 4)
local a = {}
for i = 1, (_soft and 100 or 1000) do
	a[i] = i
end
foo(table.unpack(a))
...
debug.sethook()
local res = {
	"return", -- first return (from sethook)
	"call",
	"tail call",
	"call",
	"tail call",
...
assert(a + 30000 == "add" and a - 3.0 == "sub" and a * 3.0 == "mul" and -a == "unm" and #a == "len" and a & 3 == "band")
...
do -- tests for 'source' in binary dumps
	...
	assert(h(5) == 8)
	assert(
		debug.getinfo(f).source == name -- all functions have 'source'
			and debug.getinfo(g).source == name
			and debug.getinfo(h).source == name
	)
	...
	assert(h(50) == 80)
	assert(
		debug.getinfo(f).source == "=?" -- no function has 'source'
			and debug.getinfo(g).source == "=?"
			and debug.getinfo(h).source == "=?"
	)
end
