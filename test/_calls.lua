...
function callTests.testTailCallsMetamethods()
  ...
  local function bar(x) return tbl(10, x) end
  local a, b, c = bar(100)
  t.assert.Eq(a, tbl)
  t.assert.Eq(b, 10)
  t.assert.Eq(c, 100)
  ...
  local function foo2(...) return foo1(...) end
...
function callTests.testPcallStackOverflow()
  local function loop() pcall(loop) end
...
function callTests.testTailCallChain_Call()
  ...
  t.assert.Eq(coroutine.wrap(function() return foo() end)(), 1023)
end
...
function callTests.testClosures()
  ...
  t.assert.Eq(a, 23)
  t.assert.Eq((function(x) return x * 2 end)(20), 40)
end
...
function callTests.testMultipleReturns()
  local function unlpack(j, i)
    i = i or 1
    if i <= #j then return j[i], unlpack(j, i + 1) end
  end
  ...
  local f = function() return 1, 2, 30, 4 end
  local ret2 = function(a, b) return a, b end
...
function callTests.testGenericLoad()
  ...
  a = load(function() return nil end)
  a() -- empty chunk

  t.assert.False(load(function() return true end))

  -- small bug
  local chunks = { nil, "return ", "3" }
  local f = load(function() return table.remove(chunks, 1) end)
...
function callTests.testDumpUndumpWithValues()
  ...
  t.assert.Nil(x())
  t.assert.Error(function() x("set") end, "attempt to perform arithmetic on a nil value")
end
...
function callTests.testFnLongNames()
  local tbl = { x = 1 }
  function tbl:_012345678901234567890123456789012345678901234567890123456789() return self.x end
...
function callTests.testFnParameterAdjustment()
  t.assert.Nil((function() return nil end)(4))
  t.assert.Nil((function()
    local a
    return a
  end)(4))
  t.assert.Nil((function(a) return a end)())
end
