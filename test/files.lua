...
.e+	0.e;	--;  0xX;
	...
	assert(f:read(1) == ";")
	assert(not f:read("n"))
	assert(not f:read(0)) -- end of file
	assert(f:close())
	assert(os.remove(file))
	...
	if maxint >= 2 ^ 62 then -- cannot do these tests in Small Lua
		...
		if string.packsize("i") == 4 then -- 4-byte ints
			if testerr("out-of-bound", os.date, "%Y", 2 ^ 40) then
				...
				assert(
					tonumber(os.time({ year = (1 << 31) + 1899, month = 12, day = 31, hour = 23, min = 59, sec = 59 }))
				)
