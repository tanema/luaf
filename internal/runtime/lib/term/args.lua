...
function Parser:flag(args)
  ...
  if args.long ~= nil and #args.long > 1 then self.all_flags[args.long] = flag end

  if args.short ~= nil and #args.short > 0 then self.all_flags[args.short] = flag end
...
function Parser:parse()
  ...
  while true do
    ...
    if key ~= nil then
      ...
      if definedFlag ~= nil then definedFlag:setValue(val) end
    elseif name then
...
function Parser:get(name) return self.all_flags[name] end

function Parser:printUsage()
  print(string.format("Usage: %s", self.usage))

  if self.doc ~= nil and #self.doc > 0 then print("\n" .. self.doc .. "\n") end
...
function Flag:init(args)
  ...
  local labels = {}
  if args.long ~= nil and #args.long > 0 then table.insert(labels, "--" .. args.long) end
  if args.short ~= nil and #args.short > 0 then table.insert(labels, "-" .. args.short) end
...
return {
  new = function(cli_name, usage, doc) return Parser(cli_name, usage, doc) end,
}
