...
local function fmtVal(v) return type(v) == "string" and string.format("%q", v) or tostring(v) end
...
local function customAssert(got, msg, assertMsg, ...)
  ...
  if msg ~= nil and msg ~= "" then base = base .. " (" .. tostring(msg) .. ")" end
...
local function deepEq(expected, actual)
  ...
  elseif type(expected) == "table" and type(actual) == "table" then
    for key1, value1 in pairs(expected) do
      ...
      elseif value1 ~= value2 then
        if type(value1) == "table" and type(value2) == "table" then
          if not deepEq(value1, value2) then return false end
        else
          return false
        end
      end
    ...
    for key2, _ in pairs(actual) do
      if expected[key2] == nil then return false end
    end
    return true
  end
  return false
...
function assert.Eq(expected, actual, msg)
  if deepEq(expected, actual) then return end
...
function assert.Less(actual, compare, msg) customAssert(actual < compare, msg, "expected %s < %s", actual, compare) end

function assert.LessEq(actual, compare, msg) customAssert(actual <= compare, msg, "expected %s <= %s", actual, compare) end

function assert.Greater(actual, compare, msg) customAssert(actual > compare, msg, "expected %s > %s", actual, compare) end
...
function assert.True(got, msg) customAssert(got, msg, "expected a truthy value, got %s", got) end

function assert.False(got, msg) customAssert(not got, msg, "expected false, got %s", got) end

function assert.Nil(actual, msg) customAssert(actual == nil, msg, "expected nil, got %s", actual) end

function assert.IsTable(val, msg) customAssert(type(val) == "table", msg, "expected table, got %s", type(val)) end

function assert.IsNumber(val, msg) customAssert(type(val) == "number", msg, "expected number, got %s", type(val)) end

function assert.IsString(val, msg) customAssert(type(val) == "string", msg, "expected string, got %s", type(val)) end

function assert.NotNil(actual, msg) customAssert(actual ~= nil, msg, "expected not nil value, got nil") end
...
function assert.Error(fn, errMatch, msg)
  ...
  if ok == true then customAssert(false, msg, "expected function to raise an error, got %s", err) end
...
function assert.NoError(fn, msg)
  customAssert(type(fn) == "function", nil, "bad argument #1 to NoError, should be function but received %s", type(fn))
  local ok, result = pcall(fn)
  if not ok then customAssert(false, msg, "expected function not to raise an error, but got an error: %s", result) end
  return result
end
