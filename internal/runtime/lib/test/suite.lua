...
local function runTests(cfg)
	...
	if opts.verbose then
		systemHooks = setmetatable(testHooks.verbose, { __index = systemHooks })
	end
	...
	for _, suite in ipairs(suites) do
		if util.tableCount(suite.tests) > 0 then
			runSuite(hooks, suite)
		end
	end
	local elapsed = os.clock() - startTime
	callHook(hooks.done, testResults, elapsed)
	if util.tableCount(testResults.error) + util.tableCount(testResults.fail) > 0 then
		os.exit(1)
	end
end
